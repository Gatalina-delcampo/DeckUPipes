...
public sealed class AudioSessionService : IDisposable
{
    ...
    /// <summary>Binds to the current default render endpoint and its sessions.</summary>
    private void AttachDeviceLocked()
    {
        _enumerator = (IMMDeviceEnumerator)new MMDeviceEnumeratorComObject();
        _device = _enumerator.GetDefaultAudioEndpoint(EDataFlow.Render, ERole.Multimedia);
        _deviceId = _device.GetId();
    ...
    /// <summary>Releases the endpoint and every session that belonged to it.</summary>
    private void DetachDeviceLocked()
    {
    ...
    /// <summary>True when Windows routes audio somewhere other than the endpoint we track.</summary>
    private bool DefaultDeviceChangedLocked()
    {
        if (_enumerator is null || _deviceId is null)
        {
            return true;
        }
        ...
        try
        {
            current = _enumerator.GetDefaultAudioEndpoint(EDataFlow.Render, ERole.Multimedia);
            return !string.Equals(current.GetId(), _deviceId, StringComparison.OrdinalIgnoreCase);
        }
        catch (Exception)
        {
            return true;
    ...
    public void SetVolume(AudioSession session, double linearVolume)
    {
        ...
        foreach (var runtime in GetGroupRuntimes(session))
        {
            ...
            {
                // The app or the endpoint went away mid-adjustment. The reconcile
                // pass removes it, and this must never reach the UI as an exception.
            }
        }
    }
    ...
    private void Reconcile()
    {
        lock (_lock)
        {
            ...
            if (DefaultDeviceChangedLocked())
            {
                // The session list belongs to one endpoint, so a device switch has to
                // start over rather than keep showing the old device's apps.
                DetachDeviceLocked();
            }
            ...
            {
                // The endpoint went away mid-refresh; rebuild on the next tick.
                DetachDeviceLocked();
            }
        }
